...
void pageRankParallelWorker(uintV start, uintV end, Graph &g, int max_iters, PageRankType* pr_curr, PageRankType* pr_next, std::atomic<PageRankType>* atomics, CustomBarrier& my_barrier, double &time_taken) {
    timer t1;
    t1.start();
   ...
   for (int iter = 0; iter < max_iters; iter++) {
        for (uintV u = start; u < end; u++) {
            ...
            for (uintE i = 0; i < out_degree; i++) {
                ...
                // Atomic update
                PageRankType old_val = atomics[v].load();
                while (!atomics[v].compare_exchange_weak(old_val, old_val + (pr_curr[u] / out_degree)));
            }
        }
...
void pageRankParallel(Graph &g, int max_iters, uint n_workers) {
    ...
    std::atomic<PageRankType> *atomic_prs = new std::atomic<PageRankType>[n];
    ...
    for (uint t = 0; t < n_workers; t++) {
        ...
        workers.push_back(std::thread(pageRankParallelWorker, start, end, std::ref(g), max_iters, pr_curr, pr_next, atomic_prs, std::ref(my_barrier), std::ref(times[t])));
    }
